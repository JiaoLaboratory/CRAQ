...
pipline=$(basename $0)
Usage="\nUsage:\n\t#Genome assessing using AQI:\n\t$pipline -g  Genome.fa -z  Genome.fa.size -e LRout/LR_eff.size  -c SRout/SR_putative.RE.RH  -C LRout/LR_putative.SE.SH  -d SRout/SR_sort.depth  -D LRout/LR_sort.depth  [default: -r 0.75 -p 0.4 -q 0.6 -R 0.65 -P 0.4 -Q 0.6 -f 0.1 -M 10000  -n 10 -j 1 -b F -m 1000000 -s 50000 -u T  -w 1000000 -y T -x seq.size ]"
...
while getopts "s:g:z:e:c:d:C:D:f:e:n:w:m:M:j:b:r:p:q:R:P:Q:y:x:u:o" opt
do
    case $opt in
        g)      ref_fa=$OPTARG ;;
...
rm runAQI_out/tmp.ER.region runAQI_out/SRNonmap.loc.bed
fi
...
rm -rf  ER.tmp_N.stat HR.tmp_N.stat  runAQI_out/*Report.tmp  runAQI_out/Gap_out/ runAQI_out/tmp_seq* runAQI_out/locER_out/out_final.S*R.out runAQI_out/strER_out/out_final.L*R.out runAQI_out/strER_out/*tmp runAQI_out/locER_out/*tmp* runAQI_out/*gapN.tmp
rm  runAQI_out/tmp_merged* runAQI_out/*stat
