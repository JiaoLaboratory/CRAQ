#!/usr/bin/perl
#use Data::Dumper;
if (@ARGV != 5){print "USE: this scrupt is to  caculate the Average  breakpoint_number  on the  chromosome \n $0 chrsize_file final_SL_bk.txt blocksize step effectsize_file\n";
exit 1;
}else{

my ($chrsize_file,$final_SL_bk_file,$blocksize,$step,$effectsize_file)=($ARGV[0],$ARGV[1],$ARGV[2],$ARGV[3],$ARGV[4]);
...
#                        last if($bkpos >=$block_e);
#			if( $type eq "Misjoin" &&  $bkpos>= $block_s && $bkpos<$block_e ){
...
#                           $hash{$block_s_e}{mj_normvalue}=$hash{$block_s_e}{mj_normvalue}+(1/$num);}
			   ...
			   $hash{$block_s_e}{gap_value}=$hash{$block_s_e}{gap_value}+1;
                         ...
                         if( $type =~/[err|Gap|ER|HR]/   &&  $bkpos>= $block_s && $bkpos<$block_e ){
...
for (sort keys %hash){
 ...
 #next if(not defined $sup_chr{$chr});
  ...
  print OUT  "$_\t$hash{$_}{gap_num}\t$hash{$_}{localerr_num}\t$hash{$_}{gap_normvalue}\t$hash{$_}{localerr_normvalue}\n";
}
close OUT;
#print Dumper\%hash;
...
my %chr_bk_value;
print "#Chr\tBlocksum\tEffsize\tNo.Gap\tNo.ER\tGap.avg\tER.avg\tNo.norm.Gap\tNo.norm.ER\tNorm.Gap.avg\tNorm.ER.avg\n" ;
my $total_avg;
