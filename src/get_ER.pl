...
my ($pb_ont_depfile,$cover_rate_file,$window,$win_n,$skewrate)=($ARGV[0],$ARGV[1],$ARGV[2],$ARGV[3],$ARGV[4]);
...
if($chr_now eq $chr_former) {$dep{$chr_now}{$pos_now}=$depth_now; }
 ...
 for my $key(keys %cover_rate){
  next if( not defined $dep{$key}) ;
   ...
   my ($chr,$pos,$stran,$clipnum,$alldep)=(split/===/,$line)[0,1,2,3,4];

    if($clipnum >1){$threshold=1-$skewrate;}
    if($clipnum <=1) {$threshold=0.3*(1-$skewrate);}
    for my $n(1..$win_n){
     my @left;
     my @right;
     for my $i(1..($window*$n)){ 
